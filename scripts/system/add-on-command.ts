...
export function commandScopeFor(
  command: Pick<AddOnCommand<CommandHandler>, 'permissionLevel' | 'scope'>,
): CommandScope {
  if (command.scope) return command.scope;
  if (command.permissionLevel >= commandPermissionLevels.Admin)
    return commandScopes.Admin;
  if (command.permissionLevel >= commandPermissionLevels.GameDirectors)
    return commandScopes.Prophet;
  return commandScopes.Player;
}
...
export const ADD_ON_COMMANDS_TOKEN: InjectionToken<
  AddOnCommand<CommandHandler>
> = Symbol('all the custom commands');
export const COMMAND_HANDLER_CLASSES_TOKEN: InjectionToken<
  Class<CommandHandler>
> = Symbol('all the command handler classes for dispatching at runtime');
...
export function addOnCommand<T extends CommandHandler>(
  ...
  return {
    ...addOnCommandDefaults,
    ...options,
    name: `${NAMESPACE}:${options.name}`,
  } as AddOnCommand<CommandHandler>;
}
