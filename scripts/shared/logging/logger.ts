...
export class Logger {
  ...
  private readonly settings: LogSettings = inject(LOG_SETTINGS_TOKEN);
  private readonly player = injectOptional(PLAYER_TOKEN);
  ...
  warn(...messages: any[]) {
    ...
    this.output(
      `${this.brackets(MINECOIN_GOLD + 'warn')} ${this.concat(messages)}`,
    );
  }
  ...
  private prefix() {
    const parts = [this.brackets(DARK_GRAY + String(this.system.currentTick))];
    if (this.player) {
  ...
  private output(message: string) {
    ...
    if (this.settings().logToChat) {
      try {
        ...
        console.warn(
          `can't send world message in early execution: ${fullMessage}`,
        );
      }
    }
  }
